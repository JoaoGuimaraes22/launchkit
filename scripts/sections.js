...
function showStatus() {
  ...
  const installed = detectInstalledSections(compDir, state.sections);
...
async function addFlow(rl) {
  ...
  // 3. Pick insertion position
  const pageSections = parseSectionsFromPage(pageFile);
  ...
  if (pageSections.length > 0) {
    ...
    afterIdx = posAnswer - 1;
  }

  // 4. Confirm
  const confirmed = await ask(
    rl,
    `\n  Add ${section.name} (${variant.name}) after ${afterSection ? afterSection.name : "top"}?`
  );
  ...
  // 9. Summary
  console.log(`\n✓  ${meta.componentName} added to ${pageFile}`);
  console.log(`   Dict key: "${meta.dictKey}" added to ${DICT_FILES.join(", ")}`);
...
function standardEnable(sectionName, variant, afterSection, pageSections) {
  ...
  // d. Apply collapse patches if i18n disabled
  if (!i18nActive && meta.collapsePatches) {
